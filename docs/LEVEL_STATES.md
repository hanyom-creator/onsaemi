# Lv0~Lv5 에스컬레이션 상태 전이 v1.2

- WBS 1.2.2 / 확정일 2026-09-21
- 점수 규칙은 docs/RISK_WEIGHTS.md (v1.3)를 따른다.
...
- 판정 시간 = 3초 + (대본 글자 수 ÷ 초당 4글자), 1초 단위로 올림, 최소 6초·최대 30초
  - 준비 시간과 말하는 속도는 초기값이며 실험으로 보정한다.
  - 판정 시간이 끝날 때 이미 시작된 발화가 있으면 그 발화의 최종 결과까지 포함해서 판정한다 (최대 30초 유지).
...
대본마다 다음을 저장한다: 계산된 판정 시간, 실제 판정 종료 시각, 연장 여부, 판정에 쓴 발화 번호.
...
- v1.2 | 2026-09-22 | 스트리밍 전환(온새미_API명세서_v0_2.md) 반영 — 하강 기준을 청크 수에서 시간(30초)으로 변경하고 level_reason `decay` 명시, 판정 시간 올림 단위를 2초→1초로 변경하고 진행 중 발화 포함 규칙 추가, 판정 기록(판정 시간·종료 시각·연장 여부·발화 번호) 절 신설, 세션 종료 표의 "무청크 타임아웃"을 "연결 끊김(SSE 60초 없음)"으로 변경
